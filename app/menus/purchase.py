...
def purchase_by_family(
    ...
    # --- FIX 1: Inisialisasi variabel di awal agar tidak error UnboundLocal ---
    ...
    for variant in variants:
        ...
        for option in variant["package_options"]:
            ...
            try:
                # Re-fetch decoy if needed inside loop (optional but kept from original logic)
                if use_decoy and not decoy_package_detail:                
                    decoy = DecoyInstance.get_decoy("balance")
            ...
            # --- FIX 2: Logic Penjumlahan Amount ---
            # Hanya tambahkan harga decoy jika use_decoy BENAR-BENAR True dan datanya ada
            if use_decoy and decoy_package_detail:
            ...
            try:
                res = settlement_balance(
                    ...
                    token_confirmation_idx=1 if use_decoy else 0 # Safety check index
                )
                ...
                if res and res.get("status", "") != "SUCCESS":
                    ...
                    if "Bizz-err.Amount.Total" in error_msg:
                        try:
                            ...
                            pass # Failed to parse error amount
                else:
                    successful_purchases.append(
                        f"{variant_name}|{option_order}. {option_name} - {option_price}"
                    )
                ...
                if "Bizz-err.Amount.Total" in error_msg:
                    try:
                        ...
                        if res and res.get("status", "") == "SUCCESS":
                            ...
                            if pause_on_success: print("Purchase successful!"); pause()
                            else: print("Purchase successful!")
                    except: pass
            else:
                successful_purchases.append(
